...
class NodeOSCMsgValues(bpy.types.PropertyGroup):
        ...
        osc_index: bpy.props.StringProperty(name="Argument indices. Indicate in which order the arriving arguments will be handled inside blender. Have to be in the format \'(0 [, 1, 2])\' with at least one integer, separated by a comma, and inside two parantheses(). There should be no more indices than arriving arguments, otherwise the message will be ignored and an error will be thrown", default="(0))")
